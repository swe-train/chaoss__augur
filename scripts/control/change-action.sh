#!/bin/sh
psql -U augur -h localhost -p 5432 -d padres -c "DROP MATERIALIZED VIEW augur_data.explorer_contributor_actions;
...
SELECT x.cntrb_id,
   ...
   FROM ( SELECT b.cntrb_id,
           ...
           FROM ( SELECT a.id AS cntrb_id,
                   ...
                   FROM ( SELECT issues.reporter_id AS id,
                            issues.created_at,
                            issues.repo_id,
                        ...
                        UNION ALL
                         SELECT commits.cmt_ght_author_id AS id,
                          ...
                          GROUP BY commits.cmt_commit_hash, commits.cmt_ght_author_id, commits.repo_id, (to_timestamp((commits.cmt_author_date)::text, 'YYYY-MM-DD'::text)), 'commit'::text, contributors.cntrb_login
                        UNION ALL
                        ...
                        UNION ALL
                         SELECT pull_request_events.cntrb_id AS id,
                          ...
                          WHERE ((pull_requests.pull_request_id = pull_request_events.pull_request_id) AND ((pull_request_events.action)::text = ANY (ARRAY[('merged'::character varying)::text, ('closed'::character varying)::text])))
                        UNION ALL
                        ...
                        UNION ALL
                         SELECT issues.reporter_id AS id,
                           ...
                           FROM augur_data.issues,
                            ...
                            (augur_data.message
                             LEFT JOIN augur_data.contributors ON ((contributors.cntrb_id = message.cntrb_id)))
                          WHERE ((issue_message_ref.msg_id = message.msg_id) AND (issues.issue_id = issue_message_ref.issue_id))) a,
                    augur_data.repo
                  WHERE (a.repo_id = repo.repo_id)
                  GROUP BY a.id, a.repo_id, a.action, a.created_at, repo.repo_name, a.login
